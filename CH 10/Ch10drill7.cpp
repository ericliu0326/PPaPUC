...
try
{
    ...
    if (original_points.size() != processed_points.size()) error("Something's wrong! Size of the vectors are not equal\n");
    for (int i = 0; i<original_points.size(); ++i) {
    if(original_points[i] == processed_points[i]) {}
    else error("Something's wrong! Values of the vectors are not equal");
    }
